---
title: Slidev 入门到精通
subtitle: 简洁的文案 非凡的表达
lineNumbers: true
drawings:
  persist: false
  syncAll: true
addons:
  - ./slidev_themes/addons/slidev-addon-palette
  - ./slidev_themes/addons/slidev-addon-quantide-layout
  - ./slidev_themes/addons/slidev-addon-mouse-trail-pen
  - ./slidev_themes/addons/slidev-addon-array
  - ./slidev_themes/addons/slidev-addon-interactive-table
  - ./slidev_themes/addons/slidev-addon-card
  - ./slidev_themes/addons/slidev-addon-notecell
  - slidev-addon-python-runner
  - fancy-arrow
mouseTrail:
  color: ink
python:
  installs: [ "pandas" ]
aspectRatio: 16/9
lecture: slidev 天天学
layout: cover-lecture
# background: https://cdn.jsdelivr.net/gh/zillionare/imgbed2@main/images/slidev/square/food/
# background: https://images.jieyu.ai/images/hot/me-3.png
background: https://cdn.jsdelivr.net/gh/zillionare/imgbed2@main/images/slidev/landscape/background/lab-mint.jpg
total: 60
date: now
htmlAttrs:
  class: mint
---

<!--
制作演示文稿，有很多工具可以选择。比如 powerpoint + islide，这是 windows 上的常用工具；在 mac 上，一般使用 keynotes 来制作。

这些工具很强大，但也有各种不足。动画制作过程繁琐，动画步骤一多，动画对象的排版和排序就更为复杂。对于经常要演示数学公式、或者程序代码的场景，它们更是力不从心。

对懂一点程序开发经验的人来说，如果能把文案与演示指令融合到一起，这样就有可能大大加快文稿制作速度，同时还有可能增强演示效果。

slidev 就是这样的工具。它的名字来源于 slide for developers。slide 是胶片的意思，在没有电脑之前，演示文稿就存在了，那时候它们是印制在电影胶片上，通过放映机播放的。所以，slide 一直就有演示文稿的意思。

作为一个为开发者开发的演示工具，slidev 学习起来确实有一定的难度，不过，一旦我们掌握了它，不仅会大大提高工作效率，还能大大增强视觉效果。

现在，我们就进入正题。
-->

---
title: Card Layout
layout: cards
---

## this is a card
  - icon: fa-leaf
  - orientation: portrait
  - content: 布局实现了基本的响应式设计，在小屏幕设备上会自动调整间距和元素大小，确保在各种设备上都有良好的显示效果。


---
title: 什么是 Slidev
src: pages/slidev/10.md
---

---
title: 代码演示
src: pages/slidev/13.md
---

---
title: Slidev 入门
layout: section
---

---
title: 安装和快速开始
src: pages/slidev/20.md
---

---
title: slidev基本概念
src: pages/slidev/22.md
---

---
title: slidev 最佳实践
src: pages/slidev/26.md
---

---
title: Slidev 基础语法
layout: section
---

---
title: 内置功能
src: pages/slidev/30.md
---

---
title: 常见错误
src: pages/slidev/32.md
---

---
title: 进阶篇
layout: section
---

---
title:  html 和 css
src: pages/slidev/40.md
---

---
title: Slidev 动画
src: pages/slidev/42.md
---


---
title: addons
layout: section
---

---
title: mouse trial
src: pages/slidev/50.md
---

---
title: array
src: pages/slidev/52.md
---

---
title: table
src: pages/slidev/54.md
---

---
title: Card
src: pages/slidev/56.md
---

---
title: 运行 Python
src: pages/slidev/58.md
---

---
title: fancy-arrow
src: pages/slidev/59.md
---
